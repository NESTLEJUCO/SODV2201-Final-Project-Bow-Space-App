...
// Fetch My Course Data API - JSON File Output for the logged-in user and diploma program
app.get("/api/mycourse", (req, res) => {
  // Get the logged-in user's ID from the request query parameters
  const loggedInUserID = req.query.userID;

  db.all(
    "SELECT * FROM Courses JOIN Enrolment ON Courses.CourseID = Enrolment.CourseID JOIN Users ON Users.UserID = Enrolment.UserID WHERE Enrolment.UserID = ?",
    [loggedInUserID],
    (err, rows) => {
      if (err) {
        console.error(err.message);
...
// checkStmt.get([formData.UserID, formData.CourseID], (err, rows) => {
//   if (err) {
...
app.post("/api/enrollcourse", (req, res) => {
  ...
  // Check if the user is already enrolled in the course and the enrollment limit
  const checkStmt = db.prepare(`
  ...
  checkStmt.get([formData.UserID, formData.CourseID], (checkErr, checkRows) => {
    if (checkErr) {
      console.error("Error checking enrollment:", checkErr);
      res.status(500).send("Error checking enrollment");
    } else {
      const alreadyEnrolled = checkRows.EnrolledCoursesCount > 0;
      ...
      if (alreadyEnrolled) {
        ...
        countStmt.get([formData.UserID], (countErr, countRows) => {
          if (countErr) {
            console.error("Error counting enrolled courses:", countErr);
            res.status(500).send("Error counting enrolled courses");
          } else {
            const enrolledCoursesCount = countRows.EnrolledCoursesCount;

            let courseLimit;
            if (formData.Program === "Certificate") {
              courseLimit = 1;
            } else {
              // Set a default limit if the program type is not recognized
              courseLimit = 5;
            }
            // Check if the student has reached the maximum enrollment limit (2)
            if (enrolledCoursesCount >= courseLimit) {
              res
                .status(400)
                .json({ error: "Maximum enrollment limit reached" });
            } else {
              ...
              stmt.run(
                ...
                (enrollErr) => {
                  if (enrollErr) {
                    console.error("Error enrolling course:", enrollErr);
                    res.status(500).send("Error enrolling course");
                  } else {
                    res.status(200).json({
